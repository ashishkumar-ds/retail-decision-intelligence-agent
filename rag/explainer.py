...
def build_narrative(evidence: Mapping[str, Any]) -> tuple[str, list[dict[str, str]]]:
    ...
    if latest is None:
        return (
            ...
            f"GET /recommendations once the campaign audit log lists the store.",
            citations,
        )
    ...
    if outcome.get("evidence_state") == "SUFFICIENT":
        ...
        if isinstance(causal, Mapping):
            # Robust to payload shape: P3 stores did_uplift_pct + evidence_state +
            # may store precomputed assessment_state/scale_up_eligible. If those
            # derived fields are absent (e.g. raw _causal_evidence_for_intervention
            # envelope), derive them deterministically so the narrative never
            # renders "assessment None" — 84.51/dunnhumby audits require no nulls.
