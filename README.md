...
| `/executions`, `/executions/{id}/reverse` | GET/POST | Execution journal and reversal (auth) |
| `/metrics` | GET | Prometheus metrics (auth; see ops/prometheus/) |
| `/analytics/root-causes` | GET | Root-cause tags over the decision log — off-path analytics via the dimensions API, numbers redacted before egress (auth) |
...
the token comes from `APPROVAL_AUTH_TOKEN` (or a named `APPROVAL_TOKENS`
entry: `token:user:role` with roles `approver`/`viewer`; the authenticated
principal is recorded as `decided_by` in the record and audit ledger). If no
token is configured, these endpoints refuse to serve (503) rather than allow
