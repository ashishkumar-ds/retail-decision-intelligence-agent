...
- **Forecast API** now also serves `GET /actuals/{store_id}?start_day&end_day` (observed sales, gaps are evidence) and `GET /controls/{store_id}` (matched-control DiD) — used for outcome measurement and the causal guardrail.
- **`POST /phase2/interventions/{id}/outcome`** accepts `started_day` (dataset DAY) and builds observations from `/actuals`, tagged `source="actuals_replay"`.
- **Evidence-driven scoring** adjusts confidence based on measured lift (`NEGATIVE` → `PAUSE_INTERVENTION`, `MEETS_TARGET` → +0.2, `REVIEW_ZONE` → ×0.8) and gates scale-up on `GET /controls` `did_uplift_pct >= 3% (CONFIRMED)`.
- **Phase 2 contract** `GET /audit?schema=contract` now returns only the 4 audited fields for strict validation (default `/audit` unchanged for n8n/humans).

All prior behavior is unchanged when no outcome evidence exists.
