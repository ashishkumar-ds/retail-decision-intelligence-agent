...
def test_recovery_direction_surfaces_signed_momentum():
    """The health score clamps negative recovery at 0, but the record must
    still distinguish a declining store from a flat/recovering one."""
    recovering = score_and_recommend(signal(baseline_forecast=100, current_forecast=110))
    flat = score_and_recommend(signal(baseline_forecast=100, current_forecast=100))
    declining = score_and_recommend(signal(baseline_forecast=100, current_forecast=90))
    assert recovering["recovery_direction"] == "recovering"
    assert recovering["recovery_velocity"] is not None
    assert flat["recovery_direction"] == "flat"
    assert declining["recovery_direction"] == "declining"
    assert declining["recovery_pct"] < 0  # signed value is not clamped
    # Health score itself still sits at the clamped floor for both flat and
    # declining (score is bounded & recomputable), the direction field carries
    # the sign.
    assert flat["store_health_score"] == declining["store_health_score"]
...
def test_api_endpoints(tmp_path, monkeypatch):
    ...
    response = client.post("/recommendations/run", headers=auth_headers)
...
def test_api_forecast_statuses_and_error_review_behavior(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(main, "get_store_info", lambda store_id: {"last_day": 1})
    monkeypatch.setattr(main, "get_prediction", lambda store_id, day: 100)
    assert client.post("/recommendations/run", headers=auth_headers).json()["recommendations"][0]["forecast_status"] == "AVAILABLE"

    monkeypatch.setattr(main, "get_store_info", lambda store_id: None)
    no_data = client.post("/recommendations/run", headers=auth_headers).json()["recommendations"][0]
    ...
    malformed = client.post("/recommendations/run", headers=auth_headers).json()["recommendations"][0]
...
def test_campaign_api_does_not_promote_campaign_18_or_rollout_status_to_delivery_evidence(monkeypatch):
    """Rollout/action fields are not evidence of delivery, and upstream extras
    are ignored: they must be *absent from* the normalized read model, not
    merely present in the raw payload. (Project 2 annotates runs with its own
    operational metadata; only the 4 consumed fields are trusted.)"""
    payload = {
        "total_runs": 1,
        "runs": [{
    ...
    record = runs[0]
    assert set(record) == {
        "campaign_label", "campaign_id", "campaign_provenance_status",
        "timing_window", "run_timestamp", "store_ids",
    }
    # Upstream rollout/benchmark fields never reach the read model.
    for promoted in ("action", "rollout_status", "phase", "benchmark_sales_uplift"):
